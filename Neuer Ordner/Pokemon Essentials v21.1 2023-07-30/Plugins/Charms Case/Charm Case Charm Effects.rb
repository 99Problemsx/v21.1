...
#===============================================================================#
module MenuHandlers
MenuHandlers.add(:debug_menu, :charm_case, {
  "name"        => _INTL("Charms Case"),
  "parent"      => :items_menu,
  "description" => _INTL("Opens Charm Case Menu."),
  "always_show" => false
})

MenuHandlers.add(:debug_menu, :add_menu, {
  "name"        => _INTL("Add Menu"),
  "parent"      => :charm_case,
  "description" => _INTL("For all things add."),
  "always_show" => false
})
MenuHandlers.add(:debug_menu, :remove_menu, {
  "name"        => _INTL("Remove Menu"),
  "parent"      => :charm_case,
  "description" => _INTL("For all things remove."),
  "always_show" => false
})
MenuHandlers.add(:debug_menu, :add_charm, {
  "name"        => _INTL("Add Charm"),
  "parent"      => :add_menu,
  "description" => _INTL("Adds Selected Charm"),
  "effect"      => proc {
    totalCharms = [] 
    GameData::Item.each do |item|
      if item.is_charm? 
        totalCharms.push(item.id.to_s) 
      end
    end
	
    $player.charmlist.each do |owned_charm_id|
      totalCharms.reject! { |charm| charm == owned_charm_id.to_s } 
    end

	message = _INTL("Choose a charm to add.")
    selectedCharmIndex = pbMessage(message, totalCharms, -1) 

    if selectedCharmIndex >= 0
      selectedCharmID = totalCharms[selectedCharmIndex].to_sym 
      pbGainCharm(selectedCharmID)
    end
    }
})

MenuHandlers.add(:debug_menu, :remove_charm, {
  "name"        => _INTL("Remove Charm"),
  "parent"      => :remove_menu,
  "description" => _INTL("Removes Selected Charm"),
  "effect"      => proc {
  totalCharms = []
    $player.charmlist.each do |owned_charm_id|
	  totalCharms.push(owned_charm_id.to_s)
    end
	message = _INTL("Choose a charm to remove.")
    selectedCharmIndex = pbMessage(message, totalCharms, -1) 
	
    if selectedCharmIndex >= 0
      selectedCharmID = totalCharms[selectedCharmIndex].to_sym
      pbRemoveCharm(selectedCharmID)
    end
  }
})

MenuHandlers.add(:debug_menu, :add_all_charms, {
  "name"        => _INTL("Add All Charm"),
  "parent"      => :add_menu,
  "description" => _INTL("Adds All Available Charms"),
  "effect"      => proc {
	pbGiveAllCharms
	pbMessage(_INTL("All charms have been added."))
  }
})

MenuHandlers.add(:debug_menu, :remove_all_charms, {
  "name"        => _INTL("Remove All Charms"),
  "parent"      => :remove_menu,
  "description" => _INTL("Removes All Available Charms"),
  "effect"      => proc {
    charmlist_copy = $player.charmlist.dup
    charmlist_copy.each do |owned_charm_id|
      $player.charmsActive[owned_charm_id] = false
      $player.charmlist.delete(owned_charm_id)
    end

    pbMessage(_INTL("All charms have been removed."))
  }
})

MenuHandlers.add(:debug_menu, :deactivate_charms, {
  "name"        => _INTL("Deactivate All Charms"),
  "parent"      => :remove_menu,
  "description" => _INTL("Deactivates All Active Charms"),
  "effect"      => proc {
	pbDeactivateAll
	pbMessage(_INTL("All charms have been deactivated."))
  }
})

MenuHandlers.add(:debug_menu, :add_elemental, {
  "name"        => _INTL("Add Selected Elemental Charm"),
  "parent"      => :add_menu,
  "description" => _INTL("Adds Selected Elemental Charm"),
  "effect"      => proc {
    totalCharms = [] 
    GameData::Item.each do |item|
      if item.is_echarm? 
    ...
    $player.elementCharmlist.each do |owned_charm_id|
      totalCharms.reject! { |charm| charm == owned_charm_id.to_s } 
    end

	message = _INTL("Choose an Elemental Charm to add.")
    selectedCharmIndex = pbMessage(message, totalCharms, -1) 

    if selectedCharmIndex >= 0
      selectedCharmID = totalCharms[selectedCharmIndex].to_sym
      pbGainElementCharm(selectedCharmID)
    end
  }
})

MenuHandlers.add(:debug_menu, :remove_elemental, {
  "name"        => _INTL("Remove Selected Elemental Charm"),
  "parent"      => :remove_menu,
  "description" => _INTL("Removes Selected Elemental Charm"),
  "effect"      => proc {
	totalCharms = []
    $player.elementCharmlist.each do |owned_charm_id|
      totalCharms.push(owned_charm_id.to_s) 
    end
	message = _INTL("Choose an Elemental Charm to remove.")
    selectedCharmIndex = pbMessage(message, totalCharms, -1) 

    if selectedCharmIndex >= 0
      selectedCharmID = totalCharms[selectedCharmIndex].to_sym 
      $player.elementCharmlist.delete(selectedCharmID)
	  $player.charmsActive[selectedCharmID] = false
    end
  }
})

MenuHandlers.add(:debug_menu, :add_all_elemental, {
  "name"        => _INTL("Add All Elemental Charms"),
  "parent"      => :add_menu,
  "description" => _INTL("Adds All Elemental Charms"),
  "effect"      => proc {
	pbGiveAllECharms
  }
})

MenuHandlers.add(:debug_menu, :remove_all_elemental, {
  "name"        => _INTL("Remove All Elemental Charms"),
  "parent"      => :remove_menu,
  "description" => _INTL("Removes All Elemental Charm"),
  "effect"      => proc {
    $player.elementCharmlist.each do |owned_charm_id|
	  $player.charmsActive[owned_charm_id] = false
      $player.elementCharmlist.delete(owned_charm_id)
	end
	pbMessage(_INTL("All elemental charms have been removed."))
  }
})

MenuHandlers.add(:debug_menu, :add_max, {
  "name"        => _INTL("Add Max Active Count"),
  "parent"      => :add_menu,
  "description" => _INTL("Add Max Active Count"),
  "effect"      => proc {
    pbIncMaxCharms
	pbMessage(_INTL("Max count increased! New count is {1}!", $player.maxCharms))
  }
})
MenuHandlers.add(:debug_menu, :remove_max, {
  "name"        => _INTL("Remove Max Active Count"),
  "parent"      => :remove_menu,
  "description" => _INTL("Remove Max Active Count"),
  "effect"      => proc {
    pbDecMaxCharms
	pbMessage(_INTL("Max count decreased! New count is {1}!", $player.maxCharms))
  }
})
end
...
def pbGenerateWildPokemon(species, level, isRoamer = false)
 ...
 if $player.activeCharm?(:CLOVERCHARM)
  ...
  if $player.activeCharm?(:NATURECHARM) && rand(100) < CharmCaseSettings::NATURE_CHARM
	genwildpoke.nature = GameData::Nature.get($player.activeNature)
   end
   
	if CharmCaseSettings::USE_ALL_HA
    # Key Charm has 30% chance of Wild Pokemon having their Hidden Ability.
	   if $player.activeCharm?(:KEYCHARM) && rand(100) < keyCharmHA
		  abilities = genwildpoke.getAbilityList
		# This will pull out all higher than and equal to 2 (Hidden Abilities)
		  filtered_abilities = abilities.select { |ability| ability[1] >= 2 }
		  numbers_only = filtered_abilities.map { |ability| ability[1] }

	# Will check to make sure numbers_only(HA) has numbers, if so, will give one of them, randomly selected.
		  if numbers_only.any?
			  random_ability_index = numbers_only.sample   
			  genwildpoke.ability_index = random_ability_index 
		  end
		end  
	else  
	  # Key Charm has 30% chance of Wild Pokemon having their Hidden Ability.
...
class PokemonEncounters
  def choose_wild_pokemon(enc_type, chance_rolls = 1)
   ...
   enc_list.sort! { |a, b| b[0] <=> a[0] }   # Highest probability first
	...
	#Change
	if $player.activeCharm?(:LINKCHARM)
	  $player.link_charm_data[3] = encounter[1]
	end
	#End Change
...
# ================================ Berry Charm ================================#
#==============================================================================#  
alias charm_pbPickBerry pbPickBerry
def pbPickBerry(berry, qty = 1)
  qty *= 2 if $player.activeCharm?(:BERRYCHARM)
  charm_pbPickBerry(berry, qty)
end
...
class Battle
  ...
  def pbGainEVsOne(idxParty, defeatedBattler)
    ...
    if $player.activeCharm?(:EFFORTCHARM)   
...
def wishingCharmAutoPop
 ...
 #----Generate pool of possible Pokemon, no starters(blacklist), no legendaries---#(by default)
		GameData::Species.each do |species|
			...
			if noLegendary 
				next if species.egg_groups.include?(:Undiscovered) && !excluded_pokemon.include?(species_id)
			end
...
#==============================================================================#
  def pbStart(ballcount)
	...
	if $player.activeCharm?(:CONTESTCHARM)
		...
		pbMessage(_INTL("#{balldif} Balls have been added due to Contest Charm!"))
	end
  end
...
EventHandlers.add(:on_wild_battle_end, :twin_charm_tracker,
  proc { |species, link_charm, decision|
    if activeCharm?(:LINKCHARM)
	...
	# enc_list. I.E. Plugin: Automatic Level Scaling - Automaticly evolves wild pokemon.
      ...
      if [1, 4].include?(decision) # Defeated/caught
        ...
        $player.link_charm_data = [0, 0, {}, nil] # Species, Chain Count, **Fled Species / Chain Count ** Added nil for automatic evolving pokemon
      end
    end
  }
)
...
#==============================================================================#
# =============  Crafting Charm **Requires Item Crafter UI Plus ============== #
 # =================================== OR =================================== #
# =========  Crafting Charm **Requires Simple item crafting system  ========== #
#==============================================================================#
